...
def main(args):
    if args.segmenter == 'nltk':
        segmenter = NLTKSegmenter()
    elif args.segmenter == 'scispacy':
        segmenter = ScispacySegmenter()
    
    output_file_prefix = 'sharded/pubmed'

    if not os.path.exists('sharded' + '/training'):
        os.makedirs('sharded' + '/training')
                
    if not os.path.exists('sharded' + '/test'):
        os.makedirs('sharded' + '/test')
    
    sharding = Sharding(args.input_files.split(','), output_file_prefix, args.n_training_shards, args.n_test_shards, args.fraction_test_set)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Text Sharding')
    parser.add_argument(
        '--input_files',
        type=str,
    ...
    parser.add_argument(
        '--segmenter',
        type=str,
        help='Specify the text segmenter to use',
        choices={'nltk', 'scispacy'}
    )
    parser.add_argument(
        '--n_training_shards',
        type=int,
